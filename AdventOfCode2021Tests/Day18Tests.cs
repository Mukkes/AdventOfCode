...
[[2,[[7,7],7]],[[5,8],[[9,3],[0,2]]]]
[[[[5,2],5],[8,[3,7]]],[[5,[7,5]],[4,4]]]";
            var expectedAnswer = 3993L;

            var solver = new Solver(input);
            Assert.AreEqual(expectedAnswer, solver.SolvePartTwo());
        }

        [TestMethod]
        public void TestPartClone()
        {
            var solver = new Solver();
            foreach (var pair in solver.Input)
            {
                Assert.AreEqual(pair.ToString(), pair.Clone().ToString());
            }
        }
    }
}